...
- **우리 가족**: 아내 **박혜상**(1981.11.07), 자녀로 추정되는 은혜(2014.05.31)·혜림(2017.05.02),
  결혼기념일 2012.09.07
- **본가**: 아버지·어머니 (두 분 생신은 음력), 결혼기념일 1978.01.27
- **처가**: 장인어른(아버님)·장모님(어머님) (두 분 생신은 음력), 결혼기념일 1980.04.12,
  처남 **박진호**(1984.03.23)
- **준희네**: 준희, 제수씨, 영후. `제수씨` 호칭으로 보아 준희는 손아래 남자 형제로 보이나
  본인 확인을 받지는 않았다.
...
- **음력 4분(아버지·어머니·아버님·어머님)의 실명과 출생연도**: 앱에 없어 비어 있다.
  출생연도가 없으면 나이가 계산되지 않는다. 알게 되면 `name`, `origin_year` 를 채워라.
- **은혜·혜림**: 결혼(2012) 이후 출생이라 자녀로 보이나 명시적으로 확인받지 않았다.
- **준희네 세 사람의 관계**: 위와 같이 추정만 해 둔 상태다.
